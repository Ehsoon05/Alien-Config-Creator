...
def main() -> None:
    ...
    if config.svn_panel_url:
        ...
        svn_fallback_urls = (
            (config.svn_panel_api_url,)
            if config.svn_panel_api_url
            and config.svn_panel_api_url.rstrip("/") != svn_api_url.rstrip("/")
            else ()
        )
    ...
    if config.mmd_panel_url:
        panels["mmd"] = MarzbanClient(
            config.mmd_panel_url,
            config.mmd_panel_username,
            config.mmd_panel_password,
            request_timeout_seconds=20,
            connect_timeout_seconds=5,
            verify_ssl=config.verify_ssl,
        )
